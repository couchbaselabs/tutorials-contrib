...
Another reason to store session data elsewhere is because of web farms. If session data is stored on the web server, then a single user can only interact with a single server. This is known as "sticky sessions".
