...
This tuturial is part of the main User Profile tutorial (both link:http://docs.couchbase.com/tutorials/profile-store/dotnet.html[.NET] and link:http://docs.couchbase.com/tutorials/profile-store/java.html[Java]). You should be able to follow along if you haven't gone through those tutorials yet, but reading those tutorials first will give you more context.
...
Start by link:https://github.com/couchbaselabs/tutorials-contrib/blob/master/modules/profile-store/examples/user-profile-sample.json?raw=true[downloading the user-profile-sample.json file (large JSON file)].
...
Review the cbimport documentation, specifically the link:https://docs.couchbase.com/server/current/tools/cbimport-json.html[cbimport json] documentation.
...
A quick sanity check shows that this suggestion makes sense. Our query is always examing `tenantId` and is always looking at `user` documents.
