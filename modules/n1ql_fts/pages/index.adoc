...
We start by creating a disjunction of two matches of *Star War* in a field called *original_title*, the first one is an exact match, and the second and one with has a fuzziness one 1. A *disjunction* is similar to an *OR* clause in SQL.
...
*Star Wars* was the first result and now after "improving" our search, it appears just as the 5th result. As expected, adding new fields generated more noise in the result. 
...
So far, if the user searches for *Star War* (with a missing *S*), he would still get fairly relevant results. However, the first 5 ones are not exactly the most popular Star Wars movies ever created.
...
Couchbase 6.5 allows you to create link:https://docs.couchbase.com/server/6.5/n1ql/n1ql-language-reference/userfun.html[User Defined Functions], we will use that feature to create a super naive function to manipulate our score. Here is the function we are going to use:
...
In the function above we *boost* (multiply by a number > 1) or *penalize* (multiply by a number < 1) according to what we think that makes a movie more relevant.

Now that we have our function ready, let's add this function to a library called *movies* inside Couchbase:
...
Finally, run the following command to create the User Defined Function:
...
Now we can slightly adjust the query to use our newly created function:
...
Without the *size* attribute, FTS would return +600 results to N1QL, then all those results would be evaluated by our *rankMovie* function and just the first 20 will be actually returned. As I expect that a movie in the 50th position even after our rankMovie function won't jump to the first positions, we could specify to FTS to return just the top 40 movies.
