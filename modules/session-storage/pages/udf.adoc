...
This tutorial is part of the main Session Store tutorial (both xref:aspnet.adoc[ASP.NET] and xref:java.adoc[Java]). You should be able to follow along if you haven't gone through those tutorials yet, but reading those tutorials first will give you more context.
...
For example: ASP.NET (as shown in the xref:aspnet.adoc[ASP.NET session store tutorial]) encodes session data as Base64.
...
NOTE: Spring encodes session data differently (see: http://docs.couchbase.com/tutorials/session-storage/java.html[Using Couchbase Server as a Session Store tutorial for Java/Spring]). The rest of this tutorial assumes Base64, but the same principles can be applied to other decoding/encoding methods.
...
In the xref:aspnet.adoc[ASP.NET Session Storage tutorial], there is a query to find the 10 most recent shopping carts being stored as session data:
...
* https://docs.couchbase.com/server/6.0/n1ql/n1ql-language-reference/metafun.html[BASE64_DECODE] decodes the session store object
* https://docs.couchbase.com/server/current/n1ql/n1ql-language-reference/selectclause.html#sec_Arguments[VALUE] will unwrap the object created by BASE64_DECODE
* https://docs.couchbase.com/server/current/n1ql/n1ql-language-reference/objectfun.html#object_put[OBJECT_PUT] is needed to combine the document key with the document value
...
If you have more questions about UDFs, you can always get help from humans in the https://forums.couchbase.com/c/n1ql[N1QL category on the Couchbase Forums].

Return to the parent Session Storage tutorial:

* xref:aspnet.adoc[ASP.NET]
* xref:java.adoc[Java]
